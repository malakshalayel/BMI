...
//   //late Future<DocumentSnapshot> docSnapshotinfo;
...
//     print("User ID in home screen: ${user?.uid}");
//     // docSnapshotinfo = FirebaseFirestore.instance
...
//                           intStatus =
//                               int.parse(doc["weight"]) /
//                               ((int.parse(doc["length"]) / 100) *
//                                   (int.parse(doc["length"]) / 100));
//                           print("int status/////////////// $intStatus");
//                           status = getInterpretation(intStatus!);
//                           FirebaseFirestore.instance
//                               .collection('user information')
...
//                                 dynamic rawTimestamp = doc["createdAt"];
//                                 Timestamp? timestamp;
...
//                                 final value =
//                                     gridIndex == 0
//                                         ? doc["status"] ?? "No gender"
//                                         : gridIndex == 1
//                                         ? "${doc["weight"] ?? "0"} Kg" // Weight with unit
//                                         : gridIndex == 2
//                                         ? formattedDate ??
//                                             "No height" // Heighte
//                                         : gridIndex == 3
//                                         ? "${doc["length"] ?? "0"} Cm" // Height with unit
//                                         : "";
//                                 return Container(
//                                   decoration: BoxDecoration(
//                                     color: Colors.white,
...
//                 "creator",
//                 isEqualTo: user?.uid,
//               ) // fix: use user?.uid instead of userId
...
//           debugPrint('Stream error or empty: ${snapshot.error}');
//           return _buildStatusContainer(
//             child: Text(
//               "No status available",
...
//           // final data = doc.data() as Map<String, dynamic>;
//           // final status = data['status']?.toString() ?? 'No status';
...
// REFACTORED HomeScreen.dart with fixes for "status" handling, safer field access, and UI logic separation
